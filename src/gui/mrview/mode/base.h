...
public:
  ...
  Math::Versorf orientation() const {
    if (snap_to_image()) {
      if (image())
        return Math::Versorf(
            image()->header().transform().rotation().cast<float>());
      else
  ...
  void reset_orientation() {
    ...
    if (image())
      orient =
          Math::Versorf(image()->header().transform().rotation().cast<float>());
  ...
  Eigen::Vector3f
  move_in_out_displacement(float distance,
  ...
  Eigen::Vector3f voxel_at(const Eigen::Vector3f &pos) const {
    ...
    const Eigen::Vector3f result =
        image()->transform().scanner2voxel.cast<float>() * pos;
    return result;
  }
...
class __Action__ : public QAction {
  NOMEMALIGN
public:
  __Action__(QActionGroup *parent, const char *const name,
             const char *const description, int index)
      : QAction(name, parent) {
    setCheckable(true);
...
template <class T> class Action : public __Action__ {
  NOMEMALIGN
public:
  Action(QActionGroup *parent, const char *const name,
         const char *const description, int index)
