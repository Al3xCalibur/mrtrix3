...
public:
  ...
  Eigen::Vector3f screen_to_model(const Eigen::Vector3f &x,
                                  const Eigen::Vector3f &depth) const {
  ...
  Eigen::Vector3f screen_to_model(const QPoint &x,
                                  const Eigen::Vector3f &depth) const {
    return screen_to_model(x, depth_of(depth));
  }

  Eigen::Vector3f screen_normal() const {
    return Eigen::Vector3f(iMVP(0, 2), iMVP(1, 2), iMVP(2, 2))
        .normalized()
        .eval();
  }

  Eigen::Vector3f screen_to_model_direction(float x, float y,
                                            float depth) const {
    x *= 2.0f / viewport[2];
    y *= 2.0f / viewport[3];
    Eigen::Vector3f S(iMVP(0, 0) * x + iMVP(0, 1) * y,
                      iMVP(1, 0) * x + iMVP(1, 1) * y,
                      iMVP(2, 0) * x + iMVP(2, 1) * y);
    if (MVP(3, 2))
      S /= iMVP(3, 2) * depth + iMVP(3, 3);
    return S;
  }

  Eigen::Vector3f screen_to_model_direction(const Eigen::Vector3f &dx,
                                            float x) const {
  ...
  Eigen::Vector3f screen_to_model_direction(const QPoint &dx,
                                            const Eigen::Vector3f &x) const {
    return screen_to_model_direction(dx, depth_of(x));
  }
...
public:
  Projection(GL::Area *parent, const GL::Font &font)
      : glarea(parent), font(font), crosshair(new Crosshair()),
  ...
  void set_viewport(const QWidget &frame) const {
    ...
    gl::Viewport(m * viewport[0], m * viewport[1], m * viewport[2],
                 m * viewport[3]);
  }
#else
  ...
  void render_lines(const Eigen::Vector3f &focus) const {
    crosshairOrientation->render(focus, *this);
  }

  void render_crosshairs(const Eigen::Vector3f &focus) const {
    crosshair->render(focus, *this);
  }

  void setup_render_text(float red = 1.0, float green = 1.0,
                         float blue = 0.0) const {
  ...
  void render_text_align(int x, int y, const std::string &text, int halign = 0,
                         int valign = 0) const {
  ...
  void render_text_inset(int x, int y, const std::string &text,
                         int inset = -1) const {
    QString s(text.c_str());
    if (inset < 0)
  ...
  void render_text(const std::string &text, int position, int line = 0) const {
    ...
    if (position & TopEdge)
      y = height() - 1.5 * font.metric.height() -
          line * font.metric.lineSpacing();
    else if (position & BottomEdge)
      y = font.metric.height() / 2 + line * font.metric.lineSpacing();
    else
      y = (height() - font.metric.height()) / 2 -
          line * font.metric.lineSpacing();
  ...
  void set(GL::Shader::Program &shader_program) const {
    ...
    gl::UniformMatrix4fv(gl::GetUniformLocation(shader_program, "MVP"), 1,
                         gl::FALSE_, modelview_projection());
  }
